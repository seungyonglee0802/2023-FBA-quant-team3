...
class TestQuery(unittest.TestCase):
    ...
    def test_query_with_min_diameter(self):
        ...
        expected = set(
            approach for approach in self.approaches
            if diameter_min <= approach.neo.diameter and diameter_min != float('nan')
        )
        self.assertGreater(len(expected), 0)
        ...
        #     if expected_item not in received:
        #         print (expected_item.neo.hazardous)

        for received_item in received:
            if received_item not in expected:
                print (received_item.neo.diameter)
    ...
    def test_query_with_not_hazardous(self):
        expected = set(
            approach for approach in self.approaches
            if not approach.neo.hazardous and approach.neo.hazardous != None
        )
        self.assertGreater(len(expected), 0)
